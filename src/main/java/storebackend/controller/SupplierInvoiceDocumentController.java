...
public class SupplierInvoiceDocumentController {
    ...
    /**
     * Parse-Endpunkt: OCR + strukturiertes Parsing
     * Extrahiert Text via OCR und parst ihn in strukturierte Felder
     */
    @Operation(summary = "Parse invoice with OCR and field extraction")
    @PostMapping("/documents/{documentId}/parse")
    @PreAuthorize("@storeAccessChecker.isStoreAdmin(#storeId)")
    public ResponseEntity<Map<String, Object>> parseInvoice(
            @PathVariable Long storeId,
            @PathVariable Long documentId,
            @RequestParam(defaultValue = "6") Integer psmMode
    ) {
        ...
        try {
            ...
            if ("application/pdf".equalsIgnoreCase(mimeType)) {
                ...
                if (pdfResult.status() == InvoiceParseStatus.TEXT_EXTRACTED) {
                    ...
                    ocrMetadata.put("engine", "pdfbox");
                    ocrMetadata.put("pageCount", pdfResult.pageCount());
                    ocrMetadata.put("durationMs", 0L);
                } else {
                    if (!localInvoiceOcrService.isTesseractAvailable()) {
                        return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
                                .body(Map.of("status", "FAILED", "errorMessage", "Tesseract ist auf dem Server nicht verfügbar."));
                    }

                    LocalInvoiceOcrService.OcrExtractionResult ocrResult;
                    try (InputStream ocrInput = new ByteArrayInputStream(content)) {
                        ocrResult = localInvoiceOcrService.extractTextWithOcr(ocrInput, psmMode);
                    }

                    if (ocrResult.status() == InvoiceParseStatus.OCR_COMPLETED) {
                        rawText = ocrResult.rawText();
                        ocrMetadata.put("engine", ocrResult.engine());
                        ocrMetadata.put("pageCount", ocrResult.pageCount());
                        ocrMetadata.put("durationMs", ocrResult.durationMs());
                    } else {
                        return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY)
                                .body(Map.of("status", "FAILED", "errorMessage", ocrResult.errorMessage()));
                    }
                }
                ...
                return ResponseEntity.status(HttpStatus.UNSUPPORTED_MEDIA_TYPE)
                        .body(Map.of("status", "FAILED", "errorMessage", "Dieser Dateityp wird nicht unterstützt."));
            }

            // Phase 2: Strukturiertes Parsing
            ParsedInvoiceFields parsedFields = invoiceFieldParser.parse(rawText);

            // Response zusammenstellen
            Map<String, Object> response = new LinkedHashMap<>();
            response.put("documentId", documentId);
            response.put("status", "COMPLETED");
            response.put("ocr", ocrMetadata);
            response.put("fields", Map.of(
                    "supplierName", parsedFields.supplierName(),
                    "invoiceNumber", parsedFields.invoiceNumber(),
                    "invoiceDate", parsedFields.invoiceDate(),
                    "deliveryDate", parsedFields.deliveryDate(),
                    "netAmount", parsedFields.netAmount(),
                    "taxAmount", parsedFields.taxAmount(),
                    "grossAmount", parsedFields.grossAmount(),
                    "currency", parsedFields.currency()
            ));
            response.put("confidence", parsedFields.confidence());
            response.put("warnings", parsedFields.warnings());
            ...
            log.error("Fehler beim Parsen der Rechnung", ex);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Map.of("status", "FAILED", "errorMessage", "Das Dokument konnte nicht gelesen werden."));
        }
    }
}
