...
@Component({
  ...
  <div class="rw">
        ...
        <div class="rw-hero__content">
          ...
          <div class="rw-hero__actions">
            <a *ngIf="whatsappNumber"
              ...
              {{ (isRiad ? 'riad.reserveWhatsapp' : 'restaurant.reserveWhatsapp') | translate }}
            </a>
    ...
    <!-- ════ 4) MENÜ ════ -->
    <main class="rw-menu">
      <h2 class="rw-menu__title">{{ (isRiad ? 'riad.menu' : 'restaurant.menu') | translate }}</h2>

      <div class="rw-empty" *ngIf="visibleProducts.length === 0">
        <span class="rw-empty__icon">🍽️</span>
        <p>{{ (isRiad ? 'riad.emptyMenu' : 'restaurant.emptyMenu') | translate }}</p>
      </div>
        ...
        <article class="rw-item" *ngFor="let p of visibleProducts">
          ...
          <div class="rw-item__body">
            ...
            <a *ngIf="whatsappNumber"
              ...
              {{ (isRiad ? 'riad.order' : 'restaurant.order') | translate }}
            </a>
          </div>
        </article>
      </div>
    ...
    <a *ngIf="whatsappNumber"
      ...
      {{ (isRiad ? 'riad.reserve' : 'restaurant.reserve') | translate }}
    </a>
...
export class RestaurantWarmLayoutComponent {
  ...
  /** true wenn Riad-Modus (Texte: Zimmer & Services statt Speisekarte). */
  get isRiad(): boolean {
    return (this.businessType || '').toUpperCase() === 'RIAD';
  }

  /** Default-Nachricht (Darija), wenn kein reservationWhatsappText gesetzt ist. */
  private get defaultPrefix(): string {
    return this.isRiad
      ? 'Salam, bghit nreserver chambre'
      : 'Salam, bghit nreserver / commander';
  }
